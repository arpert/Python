...
def draw_board(brd):
  ...
  for iy in range(len(brd)):
    for ix in range(len(brd[iy])-1):
      x = 0 + d * ix + .5 * d * (iy % 2)
      y = int((height - 0*d - d * iy) * sin60)
      if (brd[iy][ix] != 0):
         # pygame.draw.rect(screen, pal[brd[iy][ix] % len(pal)], (x, y, d - 1, d - 1), 0)
         pygame.draw.ellipse(screen, pal[brd[iy][ix] % len(pal)], (x, y, d - 1, d - 1), 0)
      else:
         # pygame.draw.rect(screen, background, (x, y, d - 1, d - 1), 0)
         pygame.draw.ellipse(screen, background, (x, y, d - 1, d - 1), 0)
  pygame.display.flip()
